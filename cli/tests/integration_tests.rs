...
mod tests {
    ...
    async fn test_build_command() {
        ...
        let output = Command::new("cargo")
            .args(&["run", "--", "build", source_file.to_str().unwrap()])
            .current_dir(project_path)
            .output()
            .await
    ...
    async fn test_format_command() {
        ...
        let output = Command::new("cargo")
            .args(&["run", "--", "format", source_file.to_str().unwrap()])
            .current_dir(project_path)
            .output()
            .await
    ...
    async fn test_lint_command() {
        ...
        let output = Command::new("cargo")
            .args(&["run", "--", "lint", source_file.to_str().unwrap()])
            .current_dir(project_path)
            .output()
            .await
    ...
    async fn test_init_project() {
        ...
        let output = Command::new("cargo")
            .args(&["run", "--", "init", "test-project", "--template", "basic", "--yes"])
...
mod package_tests {
    ...
    async fn test_package_add() {
        ...
        // Add a package
        let output = Command::new("cargo")
            .args(&["run", "--", "package", "add", "test-package", "--version", "1.0.0"])
            .current_dir(project_path)
            .output()
            .await
...
mod repl_tests {
    ...
    async fn test_repl_with_script() {
        ...
        let mut child = Command::new("cargo")
            .args(&["run", "--", "repl", "--script", script_file.to_str().unwrap()])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .spawn()

...
pub fn setup_builtins() -> Vec<(&'static str, Value)> {
    vec![
        ("print", Value::Builtin(BuiltinFunction {
            name: "print".to_string(),
            arity: 1,
        })),
        ("len", Value::Builtin(BuiltinFunction {
            name: "len".to_string(),
            arity: 1,
        })),
        ("type", Value::Builtin(BuiltinFunction {
            name: "type".to_string(),
            arity: 1,
        })),
        ("str", Value::Builtin(BuiltinFunction {
            name: "str".to_string(),
            arity: 1,
        })),
        ("int", Value::Builtin(BuiltinFunction {
            name: "int".to_string(),
            arity: 1,
        })),
        ("float", Value::Builtin(BuiltinFunction {
            name: "float".to_string(),
            arity: 1,
        })),
        ("bool", Value::Builtin(BuiltinFunction {
            name: "bool".to_string(),
            arity: 1,
        })),
    ]
...
fn builtin_bool(args: &[Value]) -> Result<Value, String> {
    if args.len() != 1 {
        return Err(format!("bool() takes exactly 1 argument ({} given)", args.len()));
    }

...
impl DocGenerator {
        ...
        for line in lines {
            ...
            if !in_docstring {
                if trimmed.starts_with("\"\"\"") || trimmed.starts_with("'''") {
                    in_docstring = true;
                    quote_style = if trimmed.starts_with("\"\"\"") {
                        "\"\"\""
                    } else {
                        "'''"
                    };
    ...
    fn parse_function(
        ...
        start_line: usize,
        include_private: bool,
    ) -> Result<Option<DocFunction>> {
        let line = lines[start_line];
        let trimmed = line.trim();

        // Extract function signature
        let def_pos = if trimmed.starts_with("async def ") {
            10
        } else {
            4
        };
    ...
    fn extract_function_docstring(
        ...
        while current_line < lines.len() {
            ...
            if line.starts_with("\"\"\"") || line.starts_with("'''") {
                let quote_style = if line.starts_with("\"\"\"") {
                    "\"\"\""
                } else {
                    "'''"
                };
                    ...
                    if let Some(colon_pos) = trimmed.find(':') {
                        ...
                        if let Some(open_paren) = param_part.find('(') {
                            if let Some(close_paren) = param_part.find(')') {
                                ...
                                param_type = Some(
                                    param_part[open_paren + 1..close_paren].trim().to_string(),
                                );
                            }
                        }
                    ...
                    if let Some(eq_pos) = trimmed.find(" = ") {
                        ...
                        if include_private || !var_part.starts_with('_') {
                            let property = DocProperty {
                                ...
                                prop_type: Some(
                                    self.infer_type_from_assignment(trimmed[eq_pos + 1..].trim()),
                                ),
                                description: self
                                    .extract_property_description(
    ...
    fn generate_html_module(&self, module: &DocModule) -> Result<String> {
        ...
        html.push_str(&format!(
            "    <title>{} - Nagari Documentation</title>\n",
            module.name
        ));
        html.push_str("    <link rel=\"stylesheet\" href=\"style.css\">\n");
        html.push_str("</head>\n");
    ...
    fn extract_property_description(&self, lines: &[String], start_index: usize) -> Option<String> {
        ...
        for line in lines
            .iter()
            .take((start_index + 3).min(lines.len()))
            .skip(start_index + 1)
        {
            let line = line.trim();
            if line.starts_with('#') {
    ...
    fn infer_type_from_assignment(&self, assignment: &str) -> String {
        ...
        if assignment.starts_with('"') || assignment.starts_with('\'') {
            ...
            "int".to_string()
        } else if assignment.chars().any(|c| c == '.')
            && assignment
                .chars()
